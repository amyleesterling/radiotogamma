...
| `observatory.html` | Pick any two landmarks, or type a value, and compare frequency, wavelength, period and photon energy. Focus lens, lockstep explorer, equal-power photon counts, matter / wave / senses / sky (atmospheric transmission) / redshift (one line stretched from here to the CMB, with lookback times from flat ΛCDM) / stories layers, a guided tour, an audible octave, and the octave ladder from one footstep to the Oort cloud. State lives in the URL. |
| `lab.html` + `lab/*.js` | The Light Lab: prism (with INVENT, a custom optic built from a refractive index and a shape), CD (3D with three.js and a computed wave sheet at the bottom of the dive, 2D fallback), garden-hose rainbow, build-a-sky, a draining soap film (thin-film interference), and a hologram recorded from |O + R|² and replayed through the grating equation. Predict-first quiz. Every stage drags taller or shorter. |
